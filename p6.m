...
subplot(3,1,2), 
stem(t1(1:timewin1+1),xL(1:timewin1+1),'r'),title(strcat('Downsampled by ',num2str(L),'signal (Fs=',num2str(Fs1),'Hz)')),xlabel('? me in sec'), ylabel ('Amplitude'),grid on ,legend('downsampled') 
subplot(3,1,3), 
stem(t1(1:timewin1+1),y(1:timewin1+1),'m'),title(strcat('Decimated signal M=',num2str(L),'(Fs=',num2str(Fs1),'Hz',' or ',num2str(length(y)),'samples/sec)')) ,xlabel('? me in sec'), ylabel ('Amplitude'),grid on ,legend('decimated') 
figure 
subplot(3,1,1), 
...
subplot(3,1,2)
plot(f1,fftshift(((abs(fft(xL,N))./(N/2)))),'r'),title('spectrum of downsampled signal by 2'),grid on,axis([-Fs1/2 Fs1/2 0 1])
subplot(3,1,3)
plot(f1,fftshift(((abs(fft(y,N)./(N/2))))),'m'),title('spectrum of decimated signal by 2'),grid on,axis([-Fs1/2 Fs1/2 0 1])